...
void load_data(Territory territories[DATASET_TERRITORIES_AMOUNT]) {
    ...
    for (index = 0;
         fscanf(dataset_file, " %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf ",
                ...
                &territory.unsafe_sex_yll) == columns_amount;
         index++) {
...
int solve_question_one(Territory territories[DATASET_TERRITORIES_AMOUNT], double maximum_yll) {
    ...
    if (maximum_yll <= 0) {
        ...
        for (index = 0; index < DATASET_TERRITORIES_AMOUNT; index++) {
 ...
 */
double solve_question_two(Territory territories[DATASET_TERRITORIES_AMOUNT],
                          String territory_name) {
    int index;
    int territory_index = -1;
    Territory territory;

    for (index = 0; index < DATASET_TERRITORIES_AMOUNT && territory_index == -1; index++) {
...
double solve_question_four(Territory territories[DATASET_TERRITORIES_AMOUNT], int lowest_rank,
    ...
    if (lowest_rank == 0) {
        ...
        for (index = 1; index < DATASET_TERRITORIES_AMOUNT; index++) {
...
void print_question_four_answer(FILE *results_file,
    ...
    if (average_air_pollution_yll < 0.0) {
